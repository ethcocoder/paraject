...
The local provider can be configured without code changes through `PROJECTED_AGENT_BACKEND=onnx`, `PROJECTED_AGENT_ONNX_MODEL`, and `PROJECTED_AGENT_ONNX_LABELS`. The optional server backend uses `PROJECTED_AGENT_BASE_URL`, `PROJECTED_AGENT_MODEL`, and `PROJECTED_AGENT_API_KEY`; both backends support `PROJECTED_AGENT_TIMEOUT` and `PROJECTED_AGENT_RETRIES`. The corresponding `projected-agent` CLI options expose these choices. Provider failures and malformed responses are returned as structured status values; the runtime never sends camera frames to the model. The ONNX action contract accepts an `input_ids` tensor shaped `[1, sequence]` and returns `[1, label_count]` logits, with labels mapped to the strict `{tool, arguments}` action format.
